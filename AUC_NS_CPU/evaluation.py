...
def topk_eval(score,  k, test_tensor, hot_tensor):
    ...
    discountlist = torch.tensor([1 / math.log(i + 1, 2) for i in range(1, k + 1)])   # Discount list to calculate dcg
    ...
    o = ((rec_tensor ^ test_tensor) * rec_tensor)
    u = ((rec_tensor ^ test_tensor) * test_tensor)

    fpr = (o.sum(dim=-1) / k).sum(dim=-1)
    fnr = (u.sum(dim=-1) / (label_count + 1e-8)).sum(dim=-1)


    ohr = ((o * hot_tensor).sum(dim=-1) / ((rec_tensor * hot_tensor).sum(dim=-1) + 1e-8)).sum(dim=-1)
    ocr = ((o * (~hot_tensor)).sum(dim=-1) / ((rec_tensor * (~hot_tensor)).sum(dim=-1) + 1e-8)).sum(dim=-1)
    uhr = ((u * hot_tensor).sum(dim=-1) / ((test_tensor * hot_tensor).sum(dim=-1) + 1e-8)).sum(dim=-1)
    ucr = ((u * (~hot_tensor)).sum(dim=-1) / ((test_tensor * (~hot_tensor)).sum(dim=-1) + 1e-8)).sum(dim=-1)

    # mc.show_cuda_info()
    pre.item(), recall.item(), f1.item(), ndcg.item(), ohr.item(), uhr.item(), ocr.item(), ucr.item(), fpr.item(), fnr.item()
