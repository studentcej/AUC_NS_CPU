...
def load_train_data(path, num_item):
    ...
    return train_tensor.to_sparse_coo(), prior, datapair
...
def model_test():
    ...
    sp = time.time()
    rating_mat = model.predict().cpu().detach()  # |U| * |V|
    rating_mat = erase(rating_mat)
    for k in arg.topk:
        metrices = topk_eval(rating_mat, k, test_tensor.to(device), dividing_tensor)
